...
export function SuggestionBox({ currentView = 'unknown' }) {
  ...
  const handleViewportClick = useCallback(
    (e) => {
      ...
      // Ignore clicks on our own widget chrome (FAB, panel, pins, popovers)
  ...
  useEffect(() => {
    if (!dropMode) return;
    // Capture phase so we beat any inner click handlers
    document.addEventListener('click', handleViewportClick, true);
    document.body.style.cursor = 'crosshair';
    return () => {
      document.removeEventListener('click', handleViewportClick, true);
      document.body.style.cursor = '';
    };
  }, [dropMode, handleViewportClick]);
  ...
  return (
    <>
      ...
      {dropMode && (
        ...
        >
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: C.foam }} />
          Tap anywhere to drop a comment · press Esc to cancel
        </div>
      )}

      {/* Drop-mode Escape key */}
